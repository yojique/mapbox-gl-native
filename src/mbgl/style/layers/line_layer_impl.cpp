...
bool LineLayer::Impl::evaluate(const PropertyEvaluationParameters& parameters) {
    ...
    dashLineWidth = paint.evaluate<LineWidth>(dashArrayParams).constantOr(LineWidth::defaultValue());
    ...
    passes = (paint.evaluated.get<LineOpacity>().constantOr(1.0) > 0
           ...
           && paint.evaluated.get<LineWidth>().constantOr(1) > 0)
...
float LineLayer::Impl::getLineWidth() const {
    float lineWidth = paint.evaluated.get<LineWidth>().constantOr(LineWidth::defaultValue());
